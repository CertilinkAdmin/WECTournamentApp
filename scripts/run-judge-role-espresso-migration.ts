...
async function updateJudgeRolesToEspresso() {
  try {
    console.log('🔄 Starting judge_role enum migration (HEAD → ESPRESSO)...');
    ...
    if (currentEnumValues.includes('HEAD')) {
      console.log('HEAD role found in enum. Proceeding with migration...');
      ...
      // Step 2: Create new enum with ESPRESSO and SENSORY
      await sql`CREATE TYPE judge_role AS ENUM ('ESPRESSO', 'SENSORY');`;
      console.log('Created new judge_role enum with ESPRESSO and SENSORY');
      
      // Step 3: Update table to use new enum, converting HEAD to ESPRESSO
      await sql`
        ALTER TABLE heat_judges 
        ALTER COLUMN role TYPE judge_role 
        USING CASE 
          WHEN role::text = 'HEAD' THEN 'ESPRESSO'::judge_role
          WHEN role::text = 'SENSORY' THEN 'SENSORY'::judge_role
          ELSE 'ESPRESSO'::judge_role
        END;
      `;
      console.log('Updated heat_judges table to use new enum (HEAD → ESPRESSO)');
    ...
    } else if (currentEnumValues.includes('ESPRESSO')) {
      console.log('✅ Enum already has ESPRESSO. Migration not needed.');
    } else {
