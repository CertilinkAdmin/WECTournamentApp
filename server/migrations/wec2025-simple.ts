...
async function migrateWEC2025Data() {
  try {
    ...
    // 2. Create unique competitors (avoiding duplicates)
    ...
    for (let i = 0; i < competitors.length; i++) {
      ...
      const [createdUser] = await db.insert(users).values({
        ...
        email: `${competitor.name.toLowerCase().replace(/\s+/g, '')}@wec2025.com`,
        role: 'BARISTA'
      }).returning();
    ...
    for (const judge of judges) {
      const [createdUser] = await db.insert(users).values({
        name: judge.name,
        email: `${judge.name.toLowerCase().replace(/\s+/g, '')}@wec2025.com`,
        role: 'JUDGE'
      }).returning();
    ...
    // 4. Create tournament participants
    ...
    // 6. Create key matches with actual scores
